...
void TickCntCallback( TimerHandle_t pxTimer );
...
int main( void )
{
    ...
    TimerHandle_t TickCntTimer = xTimerCreate("TickCntTimer",     // Just a text name, not used by the kernel.
                    pdMS_TO_TICKS(10),   // The timer period in ticks.
                    pdTRUE,        // The timers will auto-reload themselves when they expire.
                    ( void * ) 0,  // Assign each timer a unique id equal to its array index.
                    TickCntCallback // Each timer calls the same callback when it expires.
            );
    xTimerStart( TickCntTimer, 0 );
    ...
    for (;;) {
        ...
        if (rxframe_len > 0) {
            if (rxframe_len > MAX_FRAME_SIZE_W) {
                ...
                xil_printf("  next_output_seqnum=%d, acked_output_seqnum=%d, acking_input_seqnum=%d\r\n",
                        edma.next_output_seqnum, edma.acked_output_seqnum, edma.acking_input_seqnum);
                ...
                if ((edma.acked_output_seqnum < edma.next_output_seqnum) 
                    ...
                    edma.acked_output_seqnum = rx_acknum;
                ...
                if (rx_seqnum == ((edma.acking_input_seqnum+1) & 0xFF)) {
                    /* input seqnum is valid */
                    edma.acking_input_seqnum = rx_seqnum;

                    xil_printf("  input frame is expected new and will be ack\'ed!\r\n");
                }
        ...
        /* check for frames that must be retransmitted */
        int i;
        for (i = 0; i < TXNTAGS; ++i) {
            if ((edma.txlink.txslots[i].state == ETXSTATE_WAIT_FOR_ACK) 
                    && (current_tick - edma.txlink.txslots[i].sendtime) > ACK_WAIT_TIMEOUT) {
                /* resend the frame */
                xil_printf("Eth100_Tx: re-sending frame in slot %d: seqnum=%d\r\n",
                        i, edma.txlink.txslots[i].seqnum);
                EdmaTx_EnqueueFrameToHw(&edma.txlink, i, ++current_tick);
                /* FIXME: this is ugly hack, there should be a hw fifo for finished tags;
                 * THIS WILL BREAK IF MORE THAN ONE FRAME IS RETRANSMITTED !!  */
                /* step back the hwbottom_tag so that tag frame is recognized later */
                edma.txlink.hwbottom_tag = i-1;
                break;          /* FIXME */
            }
        }
        ...
        if (btns != prev_btns) {
            ...
            if (btns == 1) {
                ...
                xil_printf( "Eth100_Tx: creating a new frame #%d, space=%d\r\n", edma.txlink.frm_num, space);
                ...
                if ((test_frame_len_bytes/4 < space)
                    ...
                    uint32_t frame_len = FillTestFrame(tmpbuf, (test_frame_len_bytes - 12 - 2), /* in bytes */
                                            edma.next_output_seqnum, edma.acking_input_seqnum);
                    uint32_t tag = EdmaTx_CopyToHwBuffer(&edma.txlink, (void*)tmpbuf, frame_len/4, edma.next_output_seqnum);
                    EdmaTx_EnqueueFrameToHw(&edma.txlink, tag, ++current_tick);
