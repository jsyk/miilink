...
void Eth100_RX_IRQHandler(void *data)
{
	/* test */
	LocalAddress[0]++;
}
...
//	XEmacLite_EnableLoopBack(&emac);
	...
	static XScuGic_VectorTableEntry *pxVectorTable = XScuGic_ConfigTable[ XPAR_SCUGIC_SINGLE_DEVICE_ID ].HandlerTable;
	/* RX irq je 91 nebo 62 */
	pxVectorTable[91].Handler = Eth100_RX_IRQHandler;
	...
	xTaskCreate( prvTxEth100Task, ( const char * ) "Eth100Tx",	8*configMINIMAL_STACK_SIZE, NULL,	tskIDLE_PRIORITY + 1, NULL );

	xTaskCreate( prvRxEth100Task, ( const char * ) "Eth100Rx",	8*configMINIMAL_STACK_SIZE, NULL,	tskIDLE_PRIORITY + 1, NULL );
...
void prvTxEth100Task( void *pvParameters )
{
	...
	uint32_t bottom = 0;
	uint32_t frms_bottom[(1 << FRTAG_W)];
	uint32_t frm_ntag = 1;
	uint32_t top = 0;
	uint32_t frm_num = 0;
	...
	*ETH100TX_REG_STAT = 0x80000000;
	...
	for (;;) {
		...
		if (tmp_info_txframes != *ETH100TX_REG_INFO_TXFRAMES
			...
			uint32_t tag = (*ETH100TX_REG_RBOT_WFRLEN >> 2*FRBUF_MEMADDR_W) & ((1 << FRTAG_W)-1);
			bottom = frms_bottom[tag];
				...
				if (((top >= bottom) && (new_top >= bottom) && (new_top < top))
					...
					for (i = 0; i < frame_len/4; ++i) {
						...
						*FramePtr++ = dt;
						...
						FramePtr = (void*)((uint32_t)FramePtr & ((TX_FRBUF_MASK << 2) | 0xFFFF0000));
					}

					/* enqueue the frame to hardware */
					*ETH100TX_REG_RBOT_WFRLEN = (frm_ntag << (2*FRBUF_MEMADDR_W))
							| ((frame_len / 4) << FRBUF_MEMADDR_W) | (top);

					/* move the top pointer by frame length in words */
					top = (top + frame_len/4) & TX_FRBUF_MASK;

					frms_bottom[frm_ntag] = top;
					frm_ntag = (frm_ntag + 1) & ((1 << FRTAG_W)-1);

					xil_printf( "Eth100_Tx: bottom=0x%x, top=0x%x, frm_ntag=%d\r\n", bottom, top, frm_ntag);
...
void prvRxEth100Task( void *pvParameters )
{
	...
	*ETH100RX_REG_STAT = ETH100RX_RSTAT_RESET;
	/* enable interrupts */
	*ETH100RX_REG_STAT = ETH100RX_RSTAT_IRQ_EN;

	for (;;) {
		while ((*ETH100RX_REG_STAT & ETH100RX_RSTAT_NOFRAME) == 1) {
			/* pf_empty = 1 */
			vTaskDelay( 1 );
		...
		xil_printf( "Eth100Rx: rxbuf[%d..] = ", (i+bottom) & RX_FRBUF_MASK);

		/* read the frame words */
		uint8_t *rxbuf_bytes = (uint8_t*)(ETH100RX_FRBUF);

		for (i = 0; i < 4*frame_len; ++i) {
			xil_printf( "%02x:", rxbuf_bytes[(4*bottom+i) & ((RX_FRBUF_MASK << 2) | 0x03)]);
		}
		xil_printf("\r\n");

		/* move bottom by frame length */
		bottom = (bottom + frame_len) & RX_FRBUF_MASK;
