...
architecture rtl of rxlink_fsm is
    ...
    type state_t is (IDLE, DISCARD, RXPREAMBLE, RXSFD_D5, RXMACADDR, RXFRAMELEN, RXCLIDATA, RXPAD, RXFCS, VERIFYFCS); 
    ...
    type registers_t is record
        state : state_t;
        cnt : count_t;
        clidt_length : std_logic_vector(15 downto 0);       -- client data len, 11b - max client data len is 1500 Bytes (limit 2048)
        pad_length : pad_length_t;          -- calculated pad field len, 0 to 46B
    ...
    signal aux_r_state : state_t;

begin

    comb: process (r, rst, r_str_dt, r_dv, r_dt, b_overflowed)
    ...
    begin
        ...
        case v.state is
            ...
            when VERIFYFCS =>
                ...
                v.state := DISCARD;         -- and just expect end of frame, discard everything that follows
